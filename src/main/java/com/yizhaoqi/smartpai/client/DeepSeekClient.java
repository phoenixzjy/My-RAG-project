...
public class DeepSeekClient {
    ...
    public void streamResponse(String userMessage, 
        ...
        if (apiKey == null || apiKey.trim().isEmpty()) {
            onError.accept(new IllegalStateException("DeepSeek API key is not configured"));
            return;
        }
                ...
                .subscribe(
                    ...
                    error -> {
                        if (error instanceof WebClientResponseException.Unauthorized) {
                            onError.accept(new IllegalStateException("DeepSeek API key is invalid or unauthorized", error));
                        } else {
